...
- **F12.** Header declarations align (F9/B23); .cpp definitions do NOT — single space after the return type is the dominant form in core AND backends. Backends measured: ~180 single-space vs 25 padded public definitions, the padded minority clustered in DX/GL-era files and mixed WITHIN files (`bool    ImGui_ImplDX11_CreateDeviceObjects()`, `imgui_impl_dx11.cpp:436` vs `void ImGui_ImplDX11_Shutdown()`, `:672`). Definition-name padding is unsettled legacy, not a rule: new definitions single-space.
...
- **A22.** The placement matrix. Every symbol kind has exactly ONE declaration home and ONE definition home; anything unlisted resolves via A23's closing rule. imguiapp analog follows "→" per row (every imguiapp definition site is the matching `[SECTION]` region of its satellite TU).
...
- **B17.** Statics keep the full `ImGui_ImplXXX_` prefix even though static (measured license for SDK-generic utilities: B40). Uses `IM_NEW/IM_DELETE/IM_ASSERT/IM_COUNTOF/ImVector`; `nullptr` (backends switched to C++11 nullptr in 2022 — note this differs from core, §12).
...
## 18. Backend Implementation TU Authoring Specification (`backends/imgui_impl_*.{cpp,mm}` surveyed in depth)

§6's B1-B18 summarize backend conventions; §17 specifies the header; this section specifies the implementation *TU* — surveyed individually (all 20 .cpp + 2 .mm; reference files `imgui_impl_dx11.cpp` and `imgui_impl_win32.cpp` read end-to-end, the rest at multiple offsets) and collectively (measured across 21,110 implementation lines). Same v1.92.9 WIP checkout. Range: `imgui_impl_null.cpp` 102 lines (the minimum contract) to `imgui_impl_vulkan.cpp` 2,375. Continues B numbering from B32.

### 18.1 File skeleton

- **B33.** Top-of-file anatomy: byte-identical clone of the header's B19 banner + B20 checklist + 4-link footer (the B1 duplication is deliberate redundancy-for-discoverability, C4), then optionally the `// Configuration flags to add in your imconfig file:` block of commented-out `//#define IMGUI_IMPL_<SDK>_<OPT>` templates (`imgui_impl_win32.cpp:20-21`), then `// CHANGELOG`. The CHANGELOG grammar (extends B2, measured over 762 dated entries): strip-line `// (minor and older changes stripped away, please see git history for details)` (20/21 — sdlgpu3 missing, wart); entries `//  YYYY-MM-DD: [Category:] description. (#issues)` reverse-chronological; category lexicon = the SDK name (`OpenGL:` 90, `Vulkan:` 46, `DirectX12:` 25...), `Inputs:` (147), `Misc:` (68), bracket tags `[Viewports]`/`[Docking]`/`[Helpers]`; breaking entries marked `*BREAKING*` or `(Breaking)`; pending entries dated `2026-XX-XX:` at top. Two milestone entries recur near-verbatim across the corpus and date the register shifts: `2021-06-29: Reorganized backend to pull data from a single structure... (all g_XXXX access changed to bd->XXXX)` and `2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11.`
- **B34.** Directive skeleton, uniform 21/21 (unlike the headers' 16/19): `#include "imgui.h"` → `#ifndef IMGUI_DISABLE` → `#include "imgui_impl_<sdk>.h"` (+ sidecar: `imgui_impl_sdlgpu3_shaders.h`) → SDK includes under a group label (`// DirectX`, `// Vulkan includes`), symbol-commented where non-obvious (`#include <windowsx.h> // GET_X_LPARAM(), GET_Y_LPARAM()`, `imgui_impl_win32.cpp:103`). Pre-include exceptions: opengl3's `_CRT_SECURE_NO_WARNINGS` guard before everything (`imgui_impl_opengl3.cpp:127-129`); `.mm` files use `#import` (`imgui_impl_osx.mm`). SDK plumbing follows the includes: `WIN32_LEAN_AND_MEAN` guard, MSVC autolink `#pragma comment(lib, "d3dcompiler")` with reason (`imgui_impl_dx11.cpp:57-59`), `PFN_*` typedefs for dynamically-loaded APIs (`imgui_impl_win32.cpp:110-113`). EOF: last definition (or pragma pops) → `#endif // #ifndef IMGUI_DISABLE`.
- **B35.** Warning-pragma regimes SPLIT the corpus, unlike core's uniform A3 no-pop rule: 8 TUs push at top and pop at EOF (win32, glfw, sdl2, sdl3, opengl2, opengl3, sdlrenderer2, sdlrenderer3); 13 disable bare with no push/pop (all DX, vulkan, wgpu, sdlgpu3, allegro5, android, glut, null, both .mm). Both forms use the double-comment column (warning text, then `// yes, they are more terse.`, `imgui_impl_dx11.cpp:63`). Match the file you edit; new backends may pick either (the newest, null 2025-11, chose bare).

### 18.2 Section geography

- **B36.** Backends have NO `[SECTION]` system (0/21) and no index — the file is small enough to navigate by the fixed order (B37). Structure markers are two-tier: dashed BANNERS (`//----...` fences) reserved for major optional tiers with a fixed title lexicon — `MULTI-VIEWPORT / PLATFORM INTERFACE SUPPORT` (14/14 viewport-capable files, three fixed lines: title + `// This is an _advanced_ and _optional_ feature...` + `// If you are new to dear imgui or creating a new binding... completely ignore this section first..`), `FUNCTIONS` (vulkan, sdlgpu3), `SHADERS` (vulkan, wgpu), helper tiers (`Internal / Miscellaneous Vulkan Helpers`, win32's `DPI-related helpers (optional)` / `Transparency related helpers (optional)`) — and plain one-line LABELS for everything else: `// <SDK> data` (16 files), `// Forward Declarations` (15), `// Functions` (13), `// Draw callbacks` (13), `// Render function` (6). ObjC files additionally use `#pragma mark - <Title>` (`imgui_impl_metal.mm:59`).
- **B37.** Canonical definition order (the collective cardinal anchor; renderer form): banner/CHANGELOG → includes/pragmas → SDK feature-macro tier (B53) → `// <SDK> data` structs (per-texture struct, frame structs, `_Data` LAST with ctor) → GetBackendData (B39) → `// Forward Declarations` static mini-block (MV setup/teardown decls, `imgui_impl_dx11.cpp:110-112`) → `// Functions`: SetupRenderState → draw-callback trio → RenderDrawData → DestroyTexture/UpdateTexture → CreateDeviceObjects/Invalidate → Init → Shutdown → NewFrame → MULTI-VIEWPORT banner section → helper tails (VulkanH tier, win32 DPI/alpha). Platform form swaps the middle: key translator → UpdateKeyModifiers → ProcessEvent → Init family → Shutdown → Update* frame helpers (`UpdateMouseData/UpdateMouseCursor/UpdateGamepads/UpdateMonitors`) → NewFrame → MV section. Public lifecycle definitions cluster in header-declaration order per A26.

### 18.3 State & naming tiers

- **B38.** `_Data` struct anatomy (18/21 — null stateless; android/glut legacy, B41): declared under the `// <SDK> data` label; members PascalCase, column-aligned, grouped by blank lines + group comments (`// Chain GLFW callbacks: our callbacks will call the user's previously installed callbacks, if any.` heading the `PrevUserCallback*` group, `imgui_impl_glfw.cpp:264-272`); conditional members inline `#ifdef` (`#ifdef _WIN32 WNDPROC PrevWndProc;`, `:273-275`); memset ctor per B6 in 16/18 with seeded defaults after (`VertexBufferSize = 5000; IndexBufferSize = 10000;`, `imgui_impl_dx11.cpp:95`); the 2025-era pair (sdlgpu3, wgpu) uses C++11 default member initializers instead (`imgui_impl_sdlgpu3.cpp:51-80`) — tolerated era drift, memset remains canon. A `RenderState` back-pointer member carries the aligned comment `// == ImGui::GetPlatformIO().Renderer_RenderState during rendering.` (dx11/wgpu/sdlgpu3/vulkan). DX files alone keep COM-Hungarian `p` member prefixes (`pd3dDevice`, `pVB`) — SDK register, not license elsewhere.
- **B39.** GetBackendData is a fixed idiom: `static ImGui_ImplXXX_Data* ImGui_ImplXXX_GetBackendData() { return ImGui::GetCurrentContext() ? (ImGui_ImplXXX_Data*)ImGui::GetIO().Backend<Platform|Renderer>UserData : nullptr; }` preceded by the byte-cloned two-line comment `// Backend data stored in io.BackendXxxUserData to allow support for multiple Dear ImGui contexts` / `// It is STRONGLY preferred that you use docking branch with multi-viewports...` (18 sites), optionally + honesty FIXMEs (`// FIXME: multi-context support has never been tested.`, `imgui_impl_sdlgpu3.cpp:91`). Variants: window-keyed overload beside it where events arrive without a current context (`ImGui_ImplGlfw_GetBackendData(GLFWwindow*)`, `imgui_impl_glfw.cpp:302`); .mm files add one-liner `ImGui_ImplXXX_DestroyBackendData() { IM_DELETE(...); }` (`imgui_impl_osx.mm:105-106`). Conventional local: `bd` (1,451 `bd->` sites).
- **B40.** Statics naming license (the backend N12): full `ImGui_ImplXXX_` prefix is the default (~370 static definitions) — REQUIRED for anything touching io/context/bd; plain PascalCase is licensed only for SDK-generic utilities with zero imgui coupling — the complete census (29): `SafeRelease` (dx12 template `imgui_impl_dx12.cpp:290`; wgpu 11-overload set `imgui_impl_wgpu.cpp:249-313`), `Saturate` (glfw/sdl2/sdl3), `CheckShader`/`CheckProgram` (opengl3), `CreateOrResizeBuffer[s]`/`AlignBufferSize` (vulkan, sdlgpu3), `GetMachAbsoluteTimeInSeconds` (.mm pair), `GetMouseSource*`/`ConvertNSRect` (win32-extra/osx), `IsVkDown` (win32), `_IsWindowsVersionOrGreater` (win32, mimicking `<versionhelpers.h>`), `check_vk_result` (vulkan — snake_case wart, example-code vintage). Draw-callback statics are named `ImGui_ImplXXX_DrawCallback_<Name>` matching their platform_io slot.
- **B41.** Process-global license (refines B6): file-scope `static` state is legal ONLY for genuinely process-global (not per-context) facts — `g_ContextMap` + its `ImGui_ImplGlfw_ContextMap_{Add,Remove,Get}` one-liners (window→context map; rationale comment: glfwSetWindowUserPointer "is a single and shared resource", `imgui_impl_glfw.cpp:216-224`) and vulkan's `g_FunctionsLoaded` loader flag (`imgui_impl_vulkan.cpp:159-161`). android (`g_Time/g_Window/g_LogTag`, `imgui_impl_android.cpp:43-45`) and glut (`g_Time`, `imgui_impl_glut.cpp:52`) keep whole-backend `g_` state with no `_Data` struct and no already-init asserts — unmigrated pre-2021 shape, wart not license.
- **B42.** TU-local type registers: (a) shader-mirror structs are SCREAMING_SNAKE, named for the shader slot — `VERTEX_CONSTANT_BUFFER_DX10/11/12`, dx9's `CUSTOMVERTEX` + `#define D3DFVF_CUSTOMVERTEX` (`imgui_impl_dx9.cpp:73-79`), function-local `struct UBO` (`imgui_impl_sdlgpu3.cpp:130`); (b) the state-backup struct is function-local `struct BACKUP_<SDK>_STATE` instanced as `old` (`imgui_impl_dx11.cpp:240-264`); (c) SDK-dialect TU enums drop the ImGui prefix and mimic the SDK (`enum GlfwClientApi { GlfwClientApi_OpenGL, ... }`, `imgui_impl_glfw.cpp:226-231`); (d) per-texture/frame structs keep full prefixes (`ImGui_ImplDX11_Texture`, `ImGui_ImplVulkan_FrameRenderBuffers`); (e) conversion vertex types take Im-value-type form (`ImDrawVertAllegro`, `imgui_impl_allegro5.cpp:87-91`, with its `DRAW_VERT_IMGUI_TO_ALLEGRO(DST, SRC)` UPPER-param macro); (f) inside .mm, ObjC classes take the Apple register — unprefixed camelProperty `@interface MetalBuffer/FramebufferDescriptor/MetalContext` (`imgui_impl_metal.mm:62-98`), `KeyEventResponder`/`ImGuiObserver` (`imgui_impl_osx.mm:138/:270`) — EXCEPT OS-window subclasses which keep the full prefix (`ImGui_ImplOSX_Window : NSWindow`, `:894`); `/** */` doc blocks are ObjC-register-legal there (contra F17).

### 18.4 Lifecycle rituals

- **B43.** Init, exact order (reference `imgui_impl_dx11.cpp:629-670`): fetch `io` → `IMGUI_CHECKVERSION();` (20/21; null omits) → `IM_ASSERT(io.BackendXxxUserData == nullptr && "Already initialized a <platform|renderer> backend!");` (exact strings: 6 platform + 12 renderer sites) → `IM_NEW` the bd → `io.BackendXxxUserData = (void*)bd;` → `io.BackendXxxName = "imgui_impl_xxx"` (version-annotated via snprintf into a `char BackendPlatformName[32]` member when useful: `"imgui_impl_glfw (%d)", GLFW_VERSION_COMBINED`, `imgui_impl_glfw.cpp:726`; `"imgui_impl_glut (freeglut)"` fork, `imgui_impl_glut.cpp:174-176`) → `io.BackendFlags |=` one per line with aligned `// We can ...` comments → platform_io wiring (renderers: `Renderer_TextureMaxWidth/Height` + the three `DrawCallback_*` slots, `imgui_impl_dx11.cpp:643-647`; platforms: `Platform_*Fn` handlers) → SDK acquisition → `ImGui_ImplXXX_InitMultiViewportSupport()` → `return true;`.
- **B44.** Shutdown is the exact inverse plus scrub (`imgui_impl_dx11.cpp:672-690`): fetch bd → `IM_ASSERT(bd != nullptr && "No <platform|renderer> backend to shutdown, or already shutdown?");` → `ShutdownMultiViewportSupport()` → `InvalidateDeviceObjects()`/SDK release (release lines column-aligned one-liner blocks: `if (bd->pIB) { bd->pIB->Release(); bd->pIB = nullptr; }`, `:616-626`) → null the name → null the userdata → AND-out exactly the Init-set flags in ONE combined mask (`io.BackendFlags &= ~(A | B | C);`, `:687`) → `platform_io.ClearRendererHandlers()`/`ClearPlatformHandlers()` (universal since 2025-09-18) → `IM_DELETE(bd);`.
- **B45.** NewFrame: fetch bd → `IM_ASSERT(bd != nullptr && "Context or backend not initialized! Did you call ImGui_ImplXXX_Init()?");` → lazy device-object creation with the shouted failure assert (`if (!bd->pVertexShader) if (!ImGui_ImplXXX_CreateDeviceObjects()) IM_ASSERT(0 && "ImGui_ImplXXX_CreateDeviceObjects() failed!");`, `imgui_impl_dx11.cpp:692-700`). Platform NewFrame instead runs DisplaySize/scale → the `Update*` helper battery → DeltaTime.
- **B46.** Event ingest (platforms): the key translator is `ImGuiKey ImGui_ImplXXX_<KeyEvent|KeyCode|Key>ToImGuiKey(...)` — non-static WITH its prototype on the adjacent line above the definition (5 sites; the B11 "third-party code" license), a giant switch of column-aligned `case SDK_KEY: return ImGuiKey_X;` rows defaulting to `ImGuiKey_None` (legacy android/glut keep it static). `ImGui_ImplXXX_UpdateKeyModifiers` submits all four mods via `io.AddKeyEvent(ImGuiMod_*, ...)` before key events (`imgui_impl_sdl3.cpp:388-395`). `ProcessEvent` carries the cloned WantCapture usage comment above it (`:397-400`), asserts bd, then switches on event type: braced `case` blocks that `return true` (consumed) / `return false` (not ours), each block FIRST filtering by `GetViewportForWindowID(...) == nullptr`, with commented-out `//IMGUI_DEBUG_LOG(...)` probes left in place per A29(c) (`:429`, `:467-468`).
- **B47.** Undocumented-API idiom: a public-linkage function deliberately kept out of the .h is defined with its own prototype on the line directly above and the comment `// This is intentionally not declared in the .h file yet, so you will need to copy this declaration:` (`ImGui_ImplDX11_SetSwapChainDescs`, `imgui_impl_dx11.cpp:718-721`). Definitions do NOT repeat `IMGUI_IMPL_API` — the macro appears on a definition ONLY where its declaration is not visible in the TU (win32's `WndProcHandler[Ex]` whose extern lives in the header's `#if 0` block, `imgui_impl_win32.cpp:737/:747`; osx's C++-variant defs whose decls are `#ifdef`-ed out, `imgui_impl_osx.mm:427/:431`). null.cpp tags all 10 definitions — newest-file outlier, not canon.

### 18.5 Renderer canon

- **B48.** RenderDrawData measured anatomy (fills in B12; reference `imgui_impl_dx11.cpp:178-355`): `// Avoid rendering when minimized` early-out on `DisplaySize <= 0` → texture catch-up loop under the byte-cloned two-line comment (`:187-188`) → grow-with-slack buffer blocks (`TotalVtxCount + 5000` / `TotalIdxCount + 10000`) → single contiguous upload (`vtx_dst`/`idx_dst` walk) → state backup into the local `BACKUP_*_STATE old` (`// Backup DX state... (unfortunately this is very ugly looking and verbose. Close your eyes!)`, `:239`) → `SetupRenderState(draw_data, ...)` (separate static, phase-labeled `// Setup viewport` / `// Setup shader and vertex buffers` / `// Setup render state`, honesty comments on skipped backups: `// In theory we should backup and restore this as well.. very infrequently used..`, `:161-163`) → stack-local `ImGui_ImplXXX_RenderState render_state;` published `platform_io.Renderer_RenderState = bd->RenderState = &render_state;` (`:289-293`) → cmd loop with `global_idx_offset`/`global_vtx_offset`, `pcmd`, user-callback branch checking the `ResetRenderState` identifier, `clip_min`/`clip_max` projection with the `<=` skip (`:317-320`) → `platform_io.Renderer_RenderState = bd->RenderState = nullptr;` → line-by-line restore with inline releases (`:337-354`).
- **B49.** Texture trio (fills in B13): `UpdateTexture` handles `WantCreate` (asserts `TexID == ImTextureID_Invalid && BackendUserData == nullptr` and `Format == ImTextureFormat_RGBA32`; allocates an `ImGui_ImplXXX_Texture* backend_tex = IM_NEW(...)`; stores via `SetTexID`/`SetStatus(OK)`/`BackendUserData`), `WantUpdates` (iterates `tex->Updates` rects; comment notes `tex->UpdateRect` single-rect alternative), and delegates `WantDestroy && tex->UnusedFrames > 0` to a separate static `ImGui_ImplXXX_DestroyTexture` that releases, `IM_DELETE`s, clears identifiers (`// Clear identifiers and mark as destroyed (in order to allow e.g. calling InvalidateDeviceObjects while running)`) and sets `Destroyed` (`imgui_impl_dx11.cpp:357-434`). A commented-out `//IMGUI_DEBUG_LOG("UpdateTexture #%03d: ...")` probe heads WantCreate (`:379`).
- **B50.** Shaders: embedded per-API-version source strings; DX names them camelCase SDK-register locals (`static const char* vertexShader`, `imgui_impl_dx11.cpp:451`) inside `{ }` scope blocks per shader; opengl3 selects by a dashed GLSL version TABLE in the file top (`:109-125`) and per-version `#if`-style string selection; wgpu carries WGSL + SPIR-V per backend-define; sdlgpu3 alone outsources to the `_shaders.h` sidecar (B32). opengl3's debug wrapper is the model for SDK-call checking: `//#define IMGUI_IMPL_OPENGL_DEBUG` template → `#define GL_CALL(_CALL) do { _CALL; ... fprintf(stderr, ...#_CALL); } while (0) // Call with error check` / `#define GL_CALL(_CALL) _CALL // Call without error check` (`imgui_impl_opengl3.cpp:230-235`) — libc stderr IS licensed in backends for diagnostics (also `CheckShader`, android's `__android_log_print`).
- **B51.** The draw-callback trio is three one-liner statics registered in Init: `ImGui_ImplXXX_DrawCallback_ResetRenderState(const ImDrawList*, const ImDrawCmd*) {} // Intentionally empty. Used as an identifier for rendering loop to call its code. Simpler to implement this way.` + `_SetSamplerLinear`/`_SetSamplerNearest` fetching bd and poking the sampler (11 renderer files — all but sdlrenderer2, whose header lists the caveat, `imgui_impl_dx11.cpp:172-175`) — unnamed parameters, bodies on the declaration line, comment columns aligned across the trio.

### 18.6 Multi-viewport section

- **B52.** (Fills in B15.) The banner section is the file's tail before helper tiers; opens with `// Helper structure we store in the void* RendererUserData field of each ImGuiViewport to easily retrieve our backend data.` + `ImGui_ImplXXX_ViewportData` — ctor member-assigns (not memset), dtor `IM_ASSERT`s handles already freed (10 files, `imgui_impl_dx11.cpp:708-716`). Handlers are statics named exactly for their slot minus the `Platform_`/`Renderer_` prefix (census: CreateWindow/DestroyWindow 12 definitions, SetWindowSize 11, RenderWindow 11, SwapBuffers 8, full Platform_ set assigned in the 5 platform backends); `DestroyWindow` guards with the cloned comment `// The main viewport (owned by the application) will always have RendererUserData == nullptr since we didn't create the data for it.` (`:768`). Setup/teardown pair `InitMultiViewportSupport`/`ShutdownMultiViewportSupport` is forward-declared at file top (B37) and called from Init/Shutdown (B43/B44).

### 18.7 Portability & dialect

- **B53.** SDK feature-macro tier: version tests are gathered into ONE `#define` block after the includes, stated policy `// We gather version tests as define in order to easily see which features are version-dependent.` (`imgui_impl_glfw.cpp:185-214`): a `<SDK>_VERSION_COMBINED` scalar, then `<SDK>_HAS_<FEATURE> (<test>)` lines with aligned trailing comments naming the introducing version + API (`// 3.3+ glfwSetWindowOpacity`), parenthesized `(0)` hard-offs with reasons, and courtesy `#ifdef` fallbacks for mid-cycle SDK pulls (`// Let's be nice to people who pulled GLFW between...`). vulkan's X-macro loader is the same tier at scale: alphabetical `IMGUI_VULKAN_FUNC_MAP(MACRO)` + `FUNC_DEF`/`#undef` expansion (`imgui_impl_vulkan.cpp:164-247`).
- **B54.** Backends NEVER include `imgui_internal.h` (0/21). The sanctioned leak is the extern-redeclare idiom, 3 sites: `namespace ImGui { extern ImGuiIO& GetIO(ImGuiContext*); }` (`imgui_impl_glfw.cpp:296`), the win32 twin adding `GetPlatformIO(ImGuiContext*)` (`imgui_impl_win32.cpp:1440`), and wgpu's labeled block `// Dear ImGui prototypes from imgui_internal.h` + `extern ImGuiID ImHashData(...)` + a `MEMALIGN` macro annotated `(copied from IM_ALIGN() macro)` (`imgui_impl_wgpu.cpp:76-78`) — one signature at a time, provenance stated, mirroring the demo's D40 sneaky-extern.
- **B55.** Dialect census: `nullptr` 747 vs `NULL` 20 stragglers (glfw/sdl2/sdl3/win32 — warts per the 2022-10-11 changelog); FIXME 59 (`FIXME-OPT` with reference URLs, `imgui_impl_allegro5.cpp:93-95`), TODO 0, HACK 0 (F20 holds); `IM_NEW`/`IM_DELETE`/`IM_ASSERT`/`ImVector` throughout; C++11 beyond core's floor is backend-legal where the SDK demands it (`using` alias for SDK renames, `imgui_impl_wgpu.cpp:73`; range-for; default member init per B38). Dynamic loading over link dependencies: `PFN_` typedefs + LoadLibrary/dlsym-class acquisition (win32 XInput `:107-113`, glfw X11 `:277-284`), autolink pragmas only where compile-time linking is unavoidable.

### 18.8 Per-TU facts worth knowing (individual survey)

| TU | Lines | Distinguishing facts |
|---|---|---|
| `null.cpp` | 102 | Minimum contract (B18): no Data struct, no CHECKVERSION, no BackendName, `IMGUI_IMPL_API` on all defs — 4 outlier practices in one file; flags set/cleared symmetrically; texture loop inlined in RenderDrawData |
| `android.cpp` | 309 | Pre-2021 register: `g_` state, no Data/GetBackendData/asserts (B41 wart); `__android_log_print` diagnostics; static key translator |
| `glut.cpp` | 310 | Same legacy register; `256 +` key-offset trick merging chars and specials; `(freeglut)` name fork; MSVC-only pragma |
| `sdlrenderer2/3.cpp` | 312/335 | Smallest real renderers; B31 twins; no state backup (SDL_Renderer owns state) |
| `opengl2.cpp` | 418 | Fixed-pipeline reference; sampler emulation via glTexParameter (bd flags `UseTexParameterToSetSampler`/`NextSampler`) |
| `dx9.cpp` | 651 | `CUSTOMVERTEX`+`D3DFVF_CUSTOMVERTEX`; `IMGUI_COL_TO_DX9_ARGB(_COL)` BGRA-config macro pair |
| `allegro5.cpp` | 705 | Combined Platform+Renderer in one TU (both UserData slots); `ImDrawVertAllegro` + conversion macro; unindexing FIXME-OPTs with upstream URLs |
| `sdlgpu3.cpp` | 787 | Default-member-init era (B38); `PrepareDrawData` as separate public pre-pass; shader sidecar include; plain-static `CreateOrResizeBuffers` |
| `dx10/dx11.cpp` | 806/850 | B31 twins; the renderer REFERENCE anatomy (B48-B51 citations) |
| `metal.mm` | 845 | ObjC class tier (B42f); `#pragma mark`; C++ bridge wrappers via `(__bridge id<...>)`; `ImGuiViewportDataMetal` naming wart |
| `wgpu.cpp` | 1,162 | Most deviant: camelCase Data members, unprefixed `RenderResources`/`FrameResources`/`Uniforms`, `SafeRelease` overload battery; extern-ImHashData + MEMALIGN (B54); WGSL/SPIR-V dual shaders |
| `osx.mm` | 1,175 | `KeyEventResponder` (NSTextInputClient), `ImGuiObserver` (KVO), NSCursor category for undocumented cursors; `/** */` blocks; `ImGui_ImplOSX_Window` full-prefix NSWindow subclass |
| `opengl3.cpp` | 1,197 | Embedded imgl3w loader glue; GLSL version table; `GL_CALL` debug wrapper; plain-static `CheckShader/CheckProgram` |
| `sdl2/sdl3.cpp` | 1,343/1,314 | B31 twins (divergences are SDL3-API-driven); ProcessEvent reference (B46); gamepad-mode machinery; `MouseCanUseGlobalState` capability probing |
| `dx12.cpp` | 1,351 | `SafeRelease<T>` template; descriptor alloc/free through InitInfo `Fn` slots; per-frame contexts |
| `win32.cpp` | 1,548 | Platform REFERENCE; `WndProcHandler` + `WndProcHandlerEx(..., ImGuiIO&)` pair; PFN_ dynamic XInput/DPI APIs; `_IsWindowsVersionOrGreater`; DPI + alpha-compositing banner tiers |
| `glfw.cpp` | 1,727 | `GLFW_HAS_*` reference block (B53); `g_ContextMap` (B41); callback chaining `PrevUserCallback*` + `CallbacksChainForAllWindows`; Emscripten embedded/contrib-port forks; version-annotated backend name |
| `vulkan.cpp` | 2,375 | X-macro loader; `check_vk_result`; `VulkanH_` helper tier tail; `[Please zero-clear before use!]` struct contracts; `ImGuiImplVulkanFuncs_` no-underscore prefix variant (wart) |

### 18.9 Backend-implementation warts (do not imitate)

1. android + glut whole-file `g_` register (B41) — never migrated to the 2021-06-29 Data-struct reorg all other backends logged.
2. wgpu's contributed-code cluster: camelCase `_Data` members (`initInfo`, `wgpuDevice`, `pFrameResources`), unprefixed TU structs, against 20 sibling TUs (B42/B38).
3. `check_vk_result` snake_case static (`imgui_impl_vulkan.cpp:478`) and `ImGuiImplVulkanFuncs_vkCmd*` pointer names dropping the `ImGui_Impl` underscore (`:250-251`).
4. 20 `NULL` stragglers post-2022 nullptr switch (B55).
5. sdlgpu3's CHANGELOG missing the strip-line (B33); its header/impl prose still calls the API `ImGui_ImplSDLGPU_*` (§17.5 twin).
6. `// <SDK> data` label case drift: 11 lowercase `data` vs 5 `Data` (`// Allegro Data`, `// SDL Data`, `// SDL_GPU Data`, `// OpenGL Data` in opengl3 vs opengl2's `// OpenGL data`).
7. null.cpp's four-outlier bundle (B47/B43 table row) — minimum-contract file, not a template.
8. `ImGuiViewportDataMetal` suffix-form viewport struct (`imgui_impl_metal.mm:492`) vs the 10-file `ImGui_ImplXXX_ViewportData` canon.
9. The 25 padded public definitions (F12) — era residue, mixed within files.
10. dx11's `0.f` blend-factor literals (`imgui_impl_dx11.cpp:166`) — the F24 bare-`N.f` wart reaches backends.

### 18.10 imguiapp mapping (backend implementations)

`imguiapp_impl_*.cpp` binds B33-B55 directly: header-banner clone + CHANGELOG with the B33 grammar (strip-line included; categories = SDK name / `Inputs:` / `Misc:`); B34 directive skeleton with `imguiapp.h` + own header in the include slots; B36/B37 geography — no `[SECTION]`s, dashed banners only for the MV tier and major optional tiers, plain labels elsewhere, definitions in header order; ONE `_Data` struct with memset ctor + GetBackendData + the cloned multi-context comment (B38/B39); statics fully prefixed with B40's SDK-generic license; process-globals only per B41; B43-B45 rituals verbatim including exact assert strings; renderers reproduce B48-B51 (catch-up loop comment, +5000/+10000 slack, publish/null RenderState, DestroyTexture split); platform event ingest per B46; internal access only via B54's extern-redeclare with provenance comment — `imguiapp_internal.h` may be included instead ONLY because the applayer owns both files, but the backend register's default is the public header alone; dialect: `nullptr`, `FIXME-<AREA>`, no TODO/HACK, stderr diagnostics behind a `GL_CALL`-shape 0/1 debug macro.
