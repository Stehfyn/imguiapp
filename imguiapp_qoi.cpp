...
// [SECTION] Pixel ops + encode (ImGuiAppQoiPx, ImQoiEncode)
// [SECTION] Decode (ImQoiDecode)
// [SECTION] AV image-codec entry points (AppAVImageEncode / AppAVImageDecode)
...
#endif
#include "imguiapp_internal.h"   // AppAVImageEncode / AppAVImageDecode entry decls

//-----------------------------------------------------------------------------
// [SECTION] Pixel ops + encode (ImGuiAppQoiPx, ImQoiEncode)
//-----------------------------------------------------------------------------

struct ImGuiAppQoiPx
{
    unsigned char r;
    unsigned char g;
    unsigned char b;
...
static inline int ImQoiHash(const ImGuiAppQoiPx* p)
{
    return (p->r * 3 + p->g * 5 + p->b * 7 + p->a * 11) % 64;
}

static inline bool ImQoiEq(const ImGuiAppQoiPx* x, const ImGuiAppQoiPx* y)
{
    return x->r == y->r && x->g == y->g && x->b == y->b && x->a == y->a;
}

static bool ImQoiEncode(const void* rgba, int width, int height, int pitch_bytes, ImVector<char>* out)
{
    ...
    ImGuiAppQoiPx index[64] = {};                              // zero-initialized per spec
    ImGuiAppQoiPx prev = { 0, 0, 0, 255 };                     // spec-defined starting previous pixel
    int run = 0;
...
static bool ImQoiDecode(const void* qoi_bytes, int qoi_size, ImVector<char>* out_rgba, int* out_width, int* out_height)
{
    ...
    ImGuiAppQoiPx index[64] = {};
    ImGuiAppQoiPx px = { 0, 0, 0, 255 };
    int run = 0;
    int p = 14;
...
namespace ImGui
{
// The single generic image-codec entry on the ImGuiAppAV interface (imguiapp_internal.h). The
// frame-sequence encoder providers AND the flight-recorder ring both route through here; the
// ImQoi* functions above are the private QOI codec impl behind it.
bool AppAVImageEncode(const void* rgba, int width, int height, int pitch_bytes, ImVector<char>* out)
{ return ImQoiEncode(rgba, width, height, pitch_bytes, out); }
bool AppAVImageDecode(const void* bytes, int size, ImVector<char>* out_rgba, int* out_width, int* out_height)
{ return ImQoiDecode(bytes, size, out_rgba, out_width, out_height); }
} // namespace ImGui
